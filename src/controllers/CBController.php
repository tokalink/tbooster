...
class CBController extends Controller
{
    ...
    public function getIndex()
    {
        ...
        $orig_mainpath = isset($this->data['mainpath']) ? $this->data['mainpath'] : $mainpath;
    ...
    public function validation($id = null)
    {
        ...
        foreach ($this->data_inputan as $di) {
            ...
            if ($type == 'child') {
                $slug_name = \Illuminate\Support\Str::slug($di['label'], '');
        ...
        if ($validator->fails()) {
            ...
            if (Request::ajax()) {
                ...
                $res = redirect()->back()->withErrors($validator)->with([
                    'message' => cbLang('alert_validation_error', ['error' => implode(', ', $message_all)]),
                    'message_type' => 'warning',
                ])->withInput();
                \Session::driver()->save();
...
//                                 $relationship_table_pk => CRUDBooster::newId(@$ro['relationship_table']),
            ...
            if ((@$ro['type']) == 'child') {
                $name = \Illuminate\Support\Str::slug($ro['label'], '');
                $columns = $ro['columns'];
