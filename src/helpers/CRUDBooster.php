...
class CRUDBooster
{
    ...
    public static function uploadFile($name, $encrypt = false, $resize_width = null, $resize_height = null, $id = null)
    {
        if (Request::hasFile($name)) {
            ...
            if ($encrypt == true) {
                ...
                $filename = \Illuminate\Support\Str::slug($filename, '_').'.'.$ext;
            }
    ...
    public static function getSetting($name)
    {
        ...
        $content = $query ? $query->content : null;
    ...
    public static function sidebarDashboard()
    {
        ...
        if (!$menu) {
            $menu = new \stdClass();
            $menu->id = 0;
            $menu->name = cbLang('text_dashboard');
            $menu->url = self::adminPath();
            $menu->color = 'normal';
...
//     }
    ...
    public static function findPrimaryKey($table)
    {
        ...
        if (\method_exists(\Illuminate\Support\Facades\Schema::class, 'getIndexes')) {
        ...
        if (\method_exists(\Illuminate\Support\Facades\DB::connection(), 'getDoctrineSchemaManager')) {
            $pk = \Illuminate\Support\Facades\DB::getDoctrineSchemaManager()->listTableDetails($table)->getPrimaryKey();
            if(!$pk) {
                return null;
            }
