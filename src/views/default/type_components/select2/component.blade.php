@if(!empty($form['datatable']))
    ...
    @if(!empty($form['relationship_table']))
        ...
        @endpush
    @else
        @if(!empty($form['datatable_ajax']) && $form['datatable_ajax'] == true)
            ...
            @if(!empty($form['datatable']))
                @if(!empty($form['relationship_table']))
                    ...
                    if(isset($form['datatable_orig']) && $form['datatable_orig'] != ''){
                        ...
                        $dbResult = DB::table($params[0])->where($params[2], @$id ?: 0)->first();
                        $value = $dbResult ? explode(",", $dbResult->{$params[1]}) : [];
                    } else {
                        $foreignKey = CRUDBooster::getForeignKey($table, $form['relationship_table']);
                        $foreignKey2 = CRUDBooster::getForeignKey($select_table, $form['relationship_table']);
                        $value = DB::table($form['relationship_table'])->where($foreignKey, @$id ?: 0);
